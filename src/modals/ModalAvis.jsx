...
const STORAGE_KEY = "sk_team_avis";
      ...
      {[1, 2, 3, 4, 5].map(star => (
        <button
          ...
          style={{
            ...
            fontSize: 28, color: star <= (hover || value) ? "#FFD700" : "rgba(255,255,255,.2)",
...
export default function ModalAvis({ open, onClose }) {
  ...
  const [form, setForm] = useState({ nom: "", note: 0, commentaire: "", photo: null, photoPreview: null });
  ...
  const handleSubmit = (e) => {
    ...
    setForm({ nom: "", note: 0, commentaire: "", photo: null, photoPreview: null });
    setSent(true);
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 12 }}>
              ...
              <div>
                ...
                <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", padding: "10px 14px", border: "1px dashed rgba(255,255,255,.2)", background: "rgba(255,255,255,.02)" }}>
                  ...
                  <span style={{ fontFamily: "var(--fc)", fontSize: 11, color: "rgba(255,255,255,.5)" }}>{form.photo ? form.photo.name : "Choisir une photo"}</span>
              ...
              <button type="submit" className="m-submit">Publier mon avis</button>
            </form>
          )}
        </div>
